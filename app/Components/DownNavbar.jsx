...
const DownNavbar = () => {
  const { handleDown,Navbar } = useNavbar();
  return (
    <div className={`${Navbar.down?"flex":" translate-y-30 opacity-0"}  transition-all duration-600 ease-in-out bg-white  flex z-100 items-center left-1/2 -translate-x-1/2 h-11 bottom-5 text-black justify-between py-2 fixed w-[70%] rounded-md`}>
      <section className='roboto-slab-600 mx-2 text-xl font-bold flex align-middle gap-5 items-center'>
        <div onClick={(e)=>handleDown("L")} className='border-r-2 h-8 cursor-pointer flex items-center'>
      ...
      <section className='flex gap-5 mr-2 items-center'>
        <ul className='flex gap-5 mx-2 text-md'>
          <li className=''><Link href="">Projects</Link></li>
          <li><Link href="">Contact</Link></li>
        </ul>
        <div onClick={(e)=>handleDown("R")} className=' border-l-2 cursor-pointer flex items-center'>
